...
const Test = () => {
    ...
    return (
        ...
        <div className="discussion-sidebar">
            ...
            <Button variant="contained" size="small" color="secondary" onClick={handleCreate}>Create</Button>
        </div>
            ...
            {posts.map((post, index) => (
                <div key={index} className="post-card">
                    <div className="post-heading" onClick={() => handlePostClick(post.id)}>
                        <h2>{post.title}</h2>
                        <Chip label="12 comments" variant="contained" sx={{ color: '#fff', bgcolor: '#252B48', }} />
                    </div>
                    <p style={{ marginBottom: '0.5vh', }}>Author Name</p>
                    <div className="post-tags">
                        {post.tags.map((tag, idx) => (
                            <span key={idx} onClick={() => console.log(tag)}>{tag}</span>
                        ))}
                    </div>
                </div>
            ))}
